...
x_data = pd.read_excel('综合数据.xlsx', na_values = missing_values)  # 加载数据
...
y_data = pd.read_excel('y归一化.xlsx', na_values = missing_values)  # 加载数据
...
model.add(tf.keras.layers.Dense(8, activation='relu', input_shape=(8,)))   #第1层2个神经元# 现在模型就会以尺寸为 (*, 2) 的数组作为输入，# 在第一层之后，就不再需要指定输入的尺寸了：
...
#fit执行训练过程
history = model.fit(x_train, y_train, batch_size=128, epochs=2000, validation_data=(x_test, y_test), validation_freq=1)
...
pred.to_excel('20市经济损失预测值第1组.xls')
