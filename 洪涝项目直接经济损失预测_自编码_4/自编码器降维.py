...
# 6指标 （10307）数据增强 1.2 1.1 0.9 0.8 0.7 0.6 0.5 1.3 1.4 1.5
# 降维   2列
# 农作物受灾面积（公顷）	倒塌+严损房屋间数（间）	农林牧渔业损失(万元)
# 工矿商贸业损失(万元)	基础设施损失(万元)	    公共服务损失(万元)

missing_values = ["n/a", "na", "--"] #指定空数据类型,将"n/a", "na", "--"指定为NaN类型
# 加载数据
data = pd.read_excel('综合数据2.xlsx', na_values = missing_values)
...
#自编码器网络构建
input_df = Input( shape = (6, ))   #13个指标输入层为13层
x = Dense(6, activation = 'relu', name='input_layer_1')(input_df)
x = Dense(12, activation = 'relu', kernel_initializer='glorot_uniform')(x)
...
decoded = Dense(6, kernel_initializer='glorot_uniform', name='output_layer_1')(x)
