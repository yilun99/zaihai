...
x_data = pd.read_excel('潜在特征2.xlsx', na_values = missing_values)  # 加载数据
x_data.drop(['序号'], axis=1, inplace=True)   # 去掉无用的列字段
...
#数据集分出永不相见的训练集合测试集
x_train = x_data[:-10]
y_train = y_data[:-10]  #去除后10
x_test = x_data[-10:]   #后10
y_test = y_data[-10:]

#搭建网络结构
model = tf.keras.models.Sequential()
model.add(tf.keras.layers.Dense(6, activation='relu', input_shape=(6,)))   #第1层2个神经元# 现在模型就会以尺寸为 (*, 2) 的数组作为输入，# 在第一层之后，就不再需要指定输入的尺寸了：
...
#配置训练方法
model.compile(optimizer=tf.keras.optimizers.Adam(lr=0.05),   #优化器adam
...
#一次喂入数据大小batch_size，轮数epochs，validation_data规定了测试集，validation_split从训练集划分一定比例到测试集,validation_freq多少epoch测试一次
#打印出网络结构和参数统计
model.summary()
...
model.save('model_BP01.h5')  #保存model的参数

pred = model.predict(x_test)
print(pred)
pred = pd.DataFrame(pred)
pred.to_excel('20市经济损失预测值第2组.xls')
...
#导出为excel
# data1_loss.to_excel('loss.xls')
# data2_loss.to_excel('val_loss.xls')

fig = plt.figure(figsize=(7,5))       #figsize是图片的大小
ax1 = fig.add_subplot(1, 1, 1)        # ax1是子图的名字
plt.ylim((0, 0.03))  #坐标轴范围
